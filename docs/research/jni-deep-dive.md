...
- [execution-engine-deep-dive.md](execution-engine-deep-dive.md) -- JIT コンパイラと JNI の関係
...
JIT コンパイラの最も強力な最適化手法は **インライン化（Inlining）** である。インライン化とは、メソッド呼び出しを呼び出し先のコードで置き換えることで、呼び出しオーバーヘッドを除去する手法である（詳細は [execution-engine-deep-dive.md](execution-engine-deep-dive.md) セクション5.1 参照）。
...
| ファイル | 概要 |
|---------|------|
| [JniExplorer.java](../../src/main/java/JniExplorer.java) | 標準ライブラリ内の native メソッドをリフレクションで発見・可視化する |
...
| [execution-engine-deep-dive.md](execution-engine-deep-dive.md) | セクション5.1: インライン化（JNI はインライン化不可） |
| [memory-basics.md](memory-basics.md) | セクション5.2: ネイティブメソッドスタック |
| [classloader-deep-dive.md](classloader-deep-dive.md) | セクション3.3: SPI と JNI ライブラリのロード |
